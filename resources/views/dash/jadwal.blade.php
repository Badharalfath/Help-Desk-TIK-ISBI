...
@section('content')
    ...
    <body class="bg-[url('https://dummyimage.com/1920x1080/000/fff')] text-black font-sans">
        ...
        <script>
            ...
            let currentSlides = {}; // Tracks the current slide index for the first carousel
            let currentSlidesSecond = {}; // Tracks the current slide index for the second carousel

            // Function for the first carousel (before maintenance)
            ...
            // Function for the second carousel (after maintenance)
            function moveSlideSecond(jadwalId, direction) {
                const slidesSecond = document.querySelectorAll(`#carousel-second-${jadwalId} .carousel-item`);
                const totalSlidesSecond = slidesSecond.length;

                // Initialize current slide for second carousel if not already
                if (!currentSlidesSecond[jadwalId]) {
                    currentSlidesSecond[jadwalId] = 0;
                }

                // Update current slide index for second carousel
                currentSlidesSecond[jadwalId] = (currentSlidesSecond[jadwalId] + direction + totalSlidesSecond) %
                    totalSlidesSecond;

                // Hide all second slides and show the current one
                slidesSecond.forEach((slide, index) => {
                    slide.classList.add('hidden');
                    if (index === currentSlidesSecond[jadwalId]) {
                        slide.classList.remove('hidden');
                    }
                });
            }
